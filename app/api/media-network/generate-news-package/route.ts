...
import type { NewsItem, MediaBrand, ContentSource } from "@/lib/media-network/types";

// Turns an approved/reviewed news item into a content_package: caption per the
// news structure, carousel slides, reel script, hashtags, credit — plus the
// branded headline graphic. Hedging is enforced twice: in the prompt and by a
// deterministic post-check that prefixes DEVELOPING: if the model slips.
...
export async function POST(request: Request) {
  ...
  const packageType = ["breaking_news_reel", "news_carousel", "image_headline_post"].includes(String(body.package_type))
    ? String(body.package_type)
    : "image_headline_post";
  ...
  if (!item) return NextResponse.json({ success: false, error: "News item not found." }, { status: 404 });
  if (item.status === "rejected") return NextResponse.json({ success: false, error: "Item was rejected." }, { status: 400 });
  ...
  if (source) {
    ...
    if (!verdict.allowed) {
      return NextResponse.json({ success: false, error: verdict.blockers.join(" ") }, { status: 400 });
    }
  }
  if (item.sensitivity_level === "high" && item.status !== "approved") {
    return NextResponse.json(
      { success: false, error: "HIGH sensitivity item — approve it in review before generating a package." },
      { status: 400 }
    );
  }
...
- Short sentences. No corporate filler.`,
  ...
  let gen: {
    ...
    reel_script?: string | null; hashtags?: string; engagement_question?: string;
  };
  ...
  try {
    ...
    const graphic = await renderHeadlineGraphic({
      ...
      headline: String(gen.headline_graphic_text ?? item.headline).slice(0, 140),
      tag,
      creditText,
    });
  ...
  const { data: pkg, error: pkgErr } = await supabaseServer
    ...
    .insert({
      ...
      status: verdict.allowed ? "draft" : "idea",
    })
    .select("*")
    .single();
