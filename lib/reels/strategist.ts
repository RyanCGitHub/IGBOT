...
type AccountRow = { id: number; account_name: string; niche: string | null };
...
function clampBrief(raw: Record<string, unknown>): ReelBrief {
  ...
  const voiceover = typeof raw.voiceover_script === "string" && raw.voiceover_script.trim()
...
export async function generateReelBrief(account: AccountRow): Promise<{ brief: ReelBrief; personaId: number | null }> {
  ...
  return { brief: clampBrief(parsed), personaId: persona?.id ?? null };
}
